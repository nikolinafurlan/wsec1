...
export default function ListallGradessQuery({ data }) {
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);

  const handleDelete = async (id) => {
    setIsLoading(true);
    const res = await fetch(`/api/deleteGrade?id=${id}`, {
      method: 'DELETE',
    });
    setIsLoading(false);
    ...
    <NextUIProvider theme={myCustomTheme}>
      ...
      <Card css={{ h: '500px', $$cardColor: '#ffffff' }}>
       ...
       <Card css={{ h: '$24', width: '100%', $$cardColor: '$colors$primary' }}>
            <Card.Body>
            <Text h6 size={15} color="white" css={{ mt: 0 }}>
            <Link href="http://localhost:3000/listAllCourses">
            GO BACK TO COURSE PAGE  </Link>
            </Text>
            </Card.Body>
            </Card>
    </NextUIProvider>
...
export async function getServerSideProps(context) {
  const res = await fetch(`http://localhost:3000/api/listGrades`);
